module qutraj_evolve
  ...
  ! similarity transformation matrices (dense)
  double complex, allocatable :: simil_d(:,:)
  double complex, allocatable :: sinv_d(:,:)
  double complex, allocatable :: diag_hamilt(:)
  ! equally spaced time step evolution operator
  double complex, allocatable :: ests_evolve(:,:)
  double precision :: delta_t
  double complex :: alpha=1.d0, beta=0.d0
  ...
  function yevolve(t, y) ! used for diag. method
    double precision, intent(in) :: t
    double complex, intent(in) :: y(:)
    double complex :: yevolve(size(y))
    integer :: N
    N = size(y)
    if ((abs(t - delta_t) .le. epsi) .and. (delta_t .ne. -1)) then
      call zgemv('N',N,N,alpha,ests_evolve,N,y,1,beta,yevolve,1) ! use constant time step evolution
      return
    else      
      call zgemv('N',N,N,alpha,sinv_d,N,y,1,beta,yevolve,1)
      call zgemv('N',N,N,alpha,simil_d,N,(exp(diag_hamilt * t) * yevolve),1,beta,yevolve,1)
      return
    endif
  end function

  function brent(lowerlimit, upperlimit, y, mu, y_nextjump) ! used for diag. method
    ! developed based on http://en.wikipedia.org/wiki/Brent's_method
    real(wp) :: brent
    real(wp), intent(in) :: lowerlimit, upperlimit, mu
    double complex, intent(in) :: y(:)
    double complex, intent(inout) :: y_nextjump(:)
    double precision :: fa, fb, a, b, c, d, fc, s, fs, tmp, tmp2, errortol
    integer :: i
    logical :: mflag
    ...
    c = 0
    d = huge1
    y_nextjump = yevolve(a, y)
    fa = -mu + abs(braket(y_nextjump,y_nextjump))
    y_nextjump = yevolve(b, y)
    fb = -mu + abs(braket(y_nextjump,y_nextjump))

    if (fa*fb >= 0) then ! need different signs on the endpoints
      if (fa == 0) then
        brent = a
        return
      else
    ...
    elseif (abs(fa) < abs(fb)) then ! swap a,b if |fa|<|fb|
      tmp = a
      a = b
      b = tmp
      tmp = fa
    ...
    mflag = .true.
    i = 0
    do while ((fb .ne. 0.0) .and. (abs(a-b) > errortol))
      if ((fa .ne. fc) .and. (fb .ne. fc)) then
        s = a * fb * fc / (fa - fb) / (fa - fc) + b * fa * fc / (fb - fa) / (fb - fc) + c * fa * fb / (fc - fa) / (fc - fb) ! "Inverse quadratic interpolation"
      else
      ...
      if (( .not. (((s > tmp2) .and. (s < b)) .or. ((s < tmp2) .and. (s > b))) ) .or. &
         ...
         ((.not.mflag) .and. (abs(s - b) >= (abs(c - d) / 2)))) then ! apparently there is a limit to the length of a line
        s = (a + b) / 2
        mflag = .true.
      else 
        if ((mflag .and. (abs(b - c) < errorTol)) .or. (.not. mflag .and. (abs(c - d) < errorTol))) then
          s = (a + b) / 2
          mflag = .true.
        else 
          mflag = .false.
      ...
      y_nextjump = yevolve(s, y)
      fs = -mu + abs(braket(y_nextjump,y_nextjump))
      d = c
      c = b
      fc = fb
      if (fa * fs < 0) then
     ...
     i = i + 1
     if (i > 1000) then ! limit the maximum number of attemps
       print *, "exceeded the max # of attempts in brent()"
       brent = upperlimit
       return
     endif
   enddo
  ...
  subroutine evolve_diag(tout,tmax,prevjump,nextjump,y,y_tmp,y_nextjump,p,mu,nu,&
    ll_col_times,ll_col_which)
    !
    ! Evolve quantum trajectory y(t) to y(tout) using diagonalization method
    !
    double complex, intent(inout) :: y(:),y_tmp(:),y_nextjump(:)
    double precision, intent(inout) :: tout,tmax,prevjump,nextjump
    ...
    if (nextjump == 0) then ! is this the first time step?
      nextjump = brent(0.0d0, tmax - prevjump, y, mu, y_nextjump)
      mu = grnd()
    endif

    do while ((tout .ge. nextjump) .and. (nextjump .ne. tmax))
      y = y_nextjump ! use the evolved state from the last call to brent()
      ...
      do j=1,size(c_ops)
        if ((sump <= nu) .and. (nu < sump+p(j))) then
           ...
           call append(ll_col_times,nextjump)
      ...
      call normalize(y)
      prevjump = nextjump
      nextjump = nextjump + brent(0.0d0, tmax - prevjump, y, mu, y_nextjump)
      mu = grnd()
    end do

    if (tout .le. nextjump) then
      y = yevolve(tout-prevjump, y)
      prevjump = tout
      call normalize(y)
    endif
  end subroutine
