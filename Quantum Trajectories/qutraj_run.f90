...
! start matrix diagonalization stuff

  subroutine init_diag_hamiltonian(val,n)
    use qutraj_precision
    complex(wp), intent(in) :: val(n)
    integer, intent(in) :: n
    call new(diag_hamilt,val)
  end subroutine

  subroutine init_simil_dense(val, n)
    use qutraj_precision
    integer, intent(in) :: n
    complex(wp), intent(in) :: val(n,n)
    integer :: istat
    allocate(simil_d(n,n), stat=istat )
    simil_d = val
  end subroutine

  subroutine init_sinv_dense(val, n)
    use qutraj_precision
    integer, intent(in) :: n
    complex(wp), intent(in) :: val(n,n)
    integer :: istat
    allocate(sinv_d(n,n), stat=istat )
    sinv_d = val
  end subroutine

  subroutine init_ests_evolve(val, n)
    use qutraj_precision
    integer, intent(in) :: n
    complex(wp), intent(in) :: val(n)
    integer :: istat, i
    allocate(ests_evolve(n,n), stat=istat)
    ests_evolve = 0
    do i=1, n
      ests_evolve(i,i) = val(i)
    enddo
  end subroutine
...
! end
  ...
  subroutine evolve(instanceno, rngseed, show_progress)
    ...
    double precision :: t, tout, tmax, nextjump, prevjump
    double complex, allocatable :: y(:),y_tmp(:),y_nextjump(:),rho(:,:),ests_tmp(:,:)
    logical :: states
    type(operat) :: rho_sparse
    integer :: istat=0,istat2=0,traj,progress
    integer :: i,j,l,ests_dim
    !integer :: m,n
    real(wp) :: mu,nu,S
    ...
    if (allocated(diag_hamilt)) then
      unravel_type = 3 ! using matrix diagonalization
      ests_dim = size(psi0)
      allocate(y_nextjump(ests_dim)) ! this variable is used to avoid redundant calculation
      tmax = tlist(size(tlist))
      if ((tlist(2) - tlist(1)) - (tmax - tlist(size(tlist-1))) > epsi) then ! check that the time steps are equally spaced
        delta_t = tlist(2) - tlist(1)
        ! initialize the ests (delta_t) operator
        allocate(ests_tmp(ests_dim,ests_dim))
        call init_ests_evolve(exp(diag_hamilt * delta_t), ests_dim)
        call zgemm('N','N',ests_dim,ests_dim,ests_dim,alpha,simil_d,ests_dim,ests_evolve,&
                   ests_dim,beta,ests_tmp,ests_dim)
        call zgemm('N','N',ests_dim,ests_dim,ests_dim,alpha,ests_tmp,ests_dim,sinv_d,&
                   ests_dim,beta,ests_evolve,ests_dim)
        deallocate(ests_tmp)
      else
        delta_t = -1
      endif
    ...
    do traj=1,ntraj
      ...
      do i=1,size(tlist)
        ...
        case(3)
           call evolve_diag(tout,tmax,prevjump,nextjump,y,y_tmp,y_nextjump,p,mu,nu,&
            ll_col_times(traj),ll_col_which(traj)) ! working here
        case default
          call fatal_error('Unknown unravel type.')
        end select
        y_tmp = y
        if (unravel_type .ne. 3) then
          call normalize(y_tmp) ! already normalizaed with diag. method
        endif
    ...
    call finalize(y)
    call finalize(y_tmp)
    call finalize(y_nextjump)
    call finalize(p)
    if (allocated(rho)) then
      deallocate(rho)
    endif
    if (allocated(simil_d)) then
      deallocate(simil_d)
    endif
    if (allocated(sinv_d)) then
      deallocate(sinv_d)
    endif
    if (allocated(ests_evolve)) then
      deallocate(ests_evolve)
    endif
  end subroutine
