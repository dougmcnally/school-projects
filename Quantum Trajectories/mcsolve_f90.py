...
#
# Modifications made by Doug McNally for interoperability with modified
# Fortran 90 backend
###########################################################################
import numpy as np
from qutip import *
...
def mcsolve_f90(H, psi0, tlist, c_ops, e_ops, ntraj=None,
                ...
                ptrace_sel=[], calc_entropy=False, diag_method=False):
    """
	...
	diag_method : boolean
		If True the solver will attempt to use matrix diagonalization for time evolution
		in lieu of numerical integration.
    ...
    mc.diag_method = diag_method # set a flag for using matrix diagonalization technique
...
class _MC_class():
    ...
    def evolve_serial(self, args):
        ...
        if (odeconfig.e_num != 0):
            _init_e_ops()
        if (self.diag_method):
            _init_diag_hamilt() # initialize diagonalization
        # set options
...
def _init_diag_hamilt():
    d = np.size(odeconfig.psi0)
    H = scipy.sparse.csr_matrix((odeconfig.h_data,odeconfig.h_ind,odeconfig.h_ptr),(d,d)).todense()
    # build up diagonal Hamiltonian and similarity transformation matrices
    try:
        # Find the eigenstates of the effective Hamiltonian
        la, v = np.linalg.eig(H)
        # Construct the similarity transformation matricies
        S = np.matrix(v, dtype=wpc)
        Sinv = np.matrix(np.linalg.inv(S), dtype=wpc)
        Heff_diag = la # pass this into fortran with _init_hamilt        
    except: return # diagonalization failed! Do not send data to fortran
    Of = _complexarray_to_fortran(Heff_diag)
    qtf90.qutraj_run.init_diag_hamiltonian(Of, np.size(Of))
    qtf90.qutraj_run.init_simil_dense(S, d)
    qtf90.qutraj_run.init_sinv_dense(Sinv, d)
